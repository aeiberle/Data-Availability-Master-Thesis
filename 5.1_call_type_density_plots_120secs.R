...
# Filter data for time differences within 0 to +120 seconds
audio_day <- audio_day %>%
  filter(t0_timediff >= 0 & t0_timediff <= 120)
...
density_palette_calls <- c("#E41A1C", "#377EB8", "#4DAF4A", "#984EA3", "#FF7F00",   
                           "#FFFF33", "#A65628", "#F781BF", "#11999E")
...
for (specific_date in specific_dates) {
  ...
  if (nrow(audio_day_combined) > 0) {
    ...
    combined_density_plot <- ggplot(audio_day_combined, aes(x = t0_UTC, y = label_name, fill = label_name)) +
      geom_density_ridges(alpha = 1, scale = 0.45, show.legend = FALSE) +
    ...
    # ggsave(filename = paste0(output_dir, gsub("-", "", specific_date), "_density_calls_plot_combined_120s.png"), plot = combined_density_plot, width = 18, height = 9)
...
for (specific_date in specific_dates) {
  ...
  if (nrow(audio_day_combined) > 0) {
    ...
    # Save the plot for the specific date/trial
    # ggsave(filename = paste0(output_dir, gsub("-", "", specific_date), "_density_calls_position_plot_combined_120s.png"), plot = combined_density_position_plot, width = 18, height = 9)
...
if (nrow(audio_combined_ld) > 0) {
  ...
  combined_density_lead_plot <- ggplot(audio_combined_ld, aes(x = t0_timediff, y = Position, fill = Position)) +  # y = Position
    geom_density_ridges(alpha = 1, scale = 0.5, show.legend = FALSE, rel_min_height = 0.005, bandwidth = 10) +
  ...
  # ggsave(filename = paste0(output_dir, "density_calls_lead_social_categories_plot_120s.png"), plot = combined_density_lead_plot, width = 18, height = 9)
...
if (nrow(audio_combined) > 0) {
  ...
  # Save the plot for all call types across social categories
  # ggsave(filename = paste0(output_dir, "density_calls_social_categories_plot_120s.png"), plot = combined_density_social_categories_plot, width = 18, height = 9)
...
if (nrow(audio_combined_calls_total) > 0) {
  ...
  combined_density_calls_total_plot <- ggplot(audio_combined_calls_total, aes(x = t0_timediff, y = label_name, fill = label_name)) +
    geom_density_ridges(alpha = 1, scale = 0.8, show.legend = FALSE, rel_min_height = 0.005, bandwidth = 5) +
    geom_point(aes(x = t0_timediff, y = label_name), size = 1.5, alpha = 1,
               position = position_jitter(width = 0, height = 0)) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "red", linewidth = 1) +
    geom_vline(xintercept = 10, linetype = "dashed", color = "red", linewidth = 1) +
